...
def get_intermedia_state(policy_net,memory):
    ...
    _, intermediate_state = policy_net(pre_session_state_batch, state_batch_packed)
...
for i_actions in range(n_actions): # For those state-action pairs that never get update, we will use the mean Q-value of that action to fill the matrix
