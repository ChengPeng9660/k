...
if XGBRegressor is not None:
    model = XGBRegressor(
        n_estimators=int(os.environ.get("XGB_N_ESTIMATORS", "1000")),
        max_depth=int(os.environ.get("XGB_MAX_DEPTH", "7")),
    ...
    model = HistGradientBoostingRegressor(
        max_depth=int(os.environ.get("XGB_MAX_DEPTH", "7")),
        learning_rate=float(os.environ.get("XGB_ETA", "0.1")),
        max_iter=int(os.environ.get("XGB_N_ESTIMATORS", "300")),
        random_state=seed,
    )
...
# Predict the reward for different actions under different state
...
for i_action in range(n_actions):
    Q_prediction[:,i_action]=model.predict(np.append(intermedia_state_cpu,np.ones((len(intermedia_state_cpu),1))*i_action,1))
